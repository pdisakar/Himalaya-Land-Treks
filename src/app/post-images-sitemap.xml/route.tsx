...
export async function GET() {
  ...
  const filteredArray: [string, BlogImages][] = Object.entries(
    packages.blog
  ).filter(
    ([_, value]: [string, BlogImages]) => value.featured || value.banner
  );

  const urls: string[] = filteredArray.map(
    ([slug, imagesObj]: [string, BlogImages]) => {
