...
function renderHtmlNode(node: any, opts: { isList?: boolean; icon?: "check" | "uncheck" } = {}) {
  if (node.nodeType === 3) {
    // Text node
    ...
    case "P":
      return (
        <Text style={styles.paragraph}>
          {node.childNodes.map((n: any, i: number) => (
...
function renderListWithIcon(html: string, type: "check" | "uncheck") {
  if (!html) return null;
  const root = parse(html);
  return root.childNodes.map((node: any, i: number) => renderHtmlNode(node, { icon: type }));
}

export default function BrochurePDF({ data }: { data: any }) {
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.section}>
          {description.childNodes.map((node: any, i: number) => (
