...
interface PlanYourTripProps { data: any }

export default function PlanYourTrip({ data }: PlanYourTripProps) {
  const [loading, setLoading] = useState(false);
  const [filterData, setFilterData] = useState<{ categories: number[]; grade: number[]; style: number[]; title: string; duration: [number, number]; price: [number, number] }>({
    categories: [],
    grade: [],
    style: [],
    title: "",
  ...
  const handleCheckboxChange = (e: React.ChangeEvent<HTMLInputElement>) => handleCategoryChange(e);

  const fetchData = async (offset?: boolean) => {
    setLoading(true);
    try {
      let filterString = "";
      // if (filterData.title)
            ...
            <div className="plan-your-trip-container">
              ...
              <div className="common-module">
                ...
                <div className="checkbox-list">
                  <ul>
{destinations?.slice(0, 3)?.map((itm: any, idx: number) => {
                      return (
                        <li key={itm.id}>
...
{itm.children?.map((itm: any, kdx: number) => {
              ...
              <div className="common-module">
                ...
                <div className="checkbox-list">
                  <ul>
{data.categories?.trip_style?.slice(0, 10)?.map((itm: any, idx: number) => {
                      return (
                        <li key={itm.id}>
                          <div className="form-option form-check">
                            <input
