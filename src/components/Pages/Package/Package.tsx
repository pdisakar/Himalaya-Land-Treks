...
const DownloadBrochureButton = dynamic(
  () => import('@/components/BrochurePDF/DownloadBrochureButton')
);
...
export default function PackagePage({ data, url }: PackagePageProps) {
  ...
  const breadcrumbsData: BreadcrumbType[] = data?.breadcrumbs?.[0] || [];
  const total_rating =
    testimonials?.reduce((sum, itm) => sum + itm.review_rating, 0) ?? 0;
  const total_review = testimonials?.length ?? 0;
  const avrage_rating =
    total_review > 0 ? (total_rating / (total_review * 5)) * 100 : 0;

  const package_itinerary =
    itinerary?.details?.filter(i => i.itinerary_stop === 1) || [];
  const trip_testimonials = testimonials?.length
    ? testimonials
  ...
  const showItineraryMap =
    itinerary?.itinerary_map && itinerary.itinerary_map !== '';

  const Product = {
    '@context': 'https://schema.org',
    '@type': 'Product',
description: (meta?.meta_description ?? data.content.Meta?.meta_description) as string,
    mpn: `EHN${id}`,
    name: package_title,
    ...
    aggregateRating: {
      ...
      ratingValue:
        total_review > 0 ? (total_rating / total_review).toFixed(2) : '0',
  ...
  const breadcrumbSchema = {
    ...
    '@type': 'BreadcrumbList',
    itemListElement: [
      ...breadcrumbsData.map((bc: BreadcrumbType, i: number) => ({
        '@type': 'ListItem',
        position: i + 1,
        name: bc.title,
  ...
  return (
    <>
      ...
      <div className="common-box pt-0">
        <div
          className="container"
          id="navbar-sticky-container">
          {banners.length > 0 && (
            <TripGallery renderData={banners.map(a => a.media)} />
          )}
          ...
          <div className="lg:grid pt-10 package-page lg:grid-cols-10 gap-6">
            <div className="page-left lg:col-span-7">
              ...
              {package_abstract && (
                <>
                  <article
                    className="package-abstract mb-6"
                    dangerouslySetInnerHTML={{ __html: package_abstract }}
                  />
                </>
              )}
              ...
              <TripFact
                {...{
                  ...
                  accommodation,
                  package_max_altitude,
                  minPeople: pricegroup?.[0]?.min_people ?? null,
                  transportation,
                  meals,
                  additional_facts,
                }}
              ...
              {package_highlights && (
                <div className="package-section highlights ">
                  ...
                  <article
                    ...
                    dangerouslySetInnerHTML={{ __html: package_highlights }}
                  />
                </div>
              )}
              ...
              {additional_field_2 && (
                <div className="p-3 md:p-6 mb-6 lg:mb-10 bg-secondary/10 rounded-md flex gap-x-3">
                  <svg
                    className="bg-secondary flex-[0_0_24px] text-white rounded-full p-0.5"
                    height={24}
                    width={24}>
                  ...
                  <article
                    dangerouslySetInnerHTML={{
                      __html: additional_field_2,
                    }}></article>
                </div>
              )}

              <CostDetails
                package_cost_excludes={package_cost_excludes}
                package_cost_includes={package_cost_includes}
                  ...
                  <div className="common-module">
                    ...
                    <div className="relative bg-light text-headings font-medium text-md rounded-md p-6 mt-3">
                      <i className="icon h-16 w-16 opacity-20 text-secondary absolute right-6 bottom-3">
                        <svg>
                          <use
                            xlinkHref="/icons.svfvg#customize_trip"
                            fill="currentColor"
                          />
                        </svg>
                      </i>
                      <p>
                        If the provided schedule isn't suitable for you, we can
                        create personalized travel arrangements.
                      </p>
                      <Link
                        href={`/customize-trip?_trip=${id}`}
                        className="btn btn-md btn-secondary font-bold rounded-lg mt-3">
                        Customize My Trip
                      </Link>
                    </div>
                  </div>
                  ...
                  <ul className="grid gap-3 [&>li+li]:border-t [&>li+li]:border-t-border [&>li+li]:pt-5 [&>li+li]:mt-5">
                    {trip_testimonials?.map((itm: any, idx: number) => (
                      <li key={idx}>
                        <ReviewCard
                          reviewData={itm}
                          className="p-0 shadow-none border-none lg:p-0 [&>.card-body>article]:leading-[1.65]"
                        />
                      </li>
                    ))}
                  </ul>
            ...
            <aside className="page-right booking-module-sticky-top lg:col-span-3">
              {' '}
              <BookingModule
                groupPrice={pricegroup}
                price={group_default_price ?? 0}
                duration={package_duration}
                durationType={package_duration_type}
      ...
      {data.related.length > 0 && (
        <BestSellingPackages
          className="common-box border-t border-t-border"
          renderData={data.related}
          title="Similar <b>Trips</b>"
        />
      )}
    </>
  );
