...
type TripDepartureProps = {
  ...
  group_default_price: number | 0 | null;
  title: string;
  durationType?: string;
};
...
const [pricePP, setPricePP] = useState<number>(group_default_price ?? 0);
  ...
  useEffect(() => {
    ...
    setPricePP(matchingPrice || (group_default_price ?? 0));
              ...
              <div className="depature-date relative max-w-[500px]">
                <div id="datepicker" className="custom-date-picker z-10">
                  <DatePicker
                    name="date_of_birth"
                    selected={startDate}
                    onChange={(update: Date | null) => update && setStartDate(update)}
