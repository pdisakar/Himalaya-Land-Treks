...
const DatePicker = dynamic<any>(() => import("react-datepicker"));
...
type TravellerForm = {
  ...
  passport_image: File | null | string | undefined;
  passport_no: string;
  gender?: string;
};
...
type BookingFormValues = {
  ...
  airport_dropoff?: string;
  travel_insurance?: string;
  special_requirement?: string;
};

export default function TripBooking({ ip, optionsData, eDate, sDate, min_people, default_trip_id }: any) {
  ...
  let number_of_traveller = (router as any)?.query?.traveller
  ...
  let { fields, append, remove } = useFieldArray<BookingFormValues>({
    control,
    name: "travellers",
  });
...
const [findus, setFindUs] = useState<string | undefined>();
  const [customFindus, setCustomFindUs] = useState<string | undefined>();
  const [uiLoading, setUiLoading] = useState(false);
  const [loading, setLoading] = useState(false);
  const [message, setMessage] = useState("");
  const [messageType, setMessageType] = useState<"success" | "danger" | undefined>();
  ...
  const packageName = packages.filter(
    (a: any) => a.id == parseFloat(watch("trip_id"))
  )[0]?.title;
  ...
  const onSubmit = async (formData: BookingFormValues) => {
    ...
    await grecaptcha
      ...
      .then((token) => {
        (data as any).captcha_response = token;
        axios
...
{packages?.map((itm: any, idx: number) => {
                                  return (
