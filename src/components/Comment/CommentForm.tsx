...
export default function CommentForm({ title, classes, subTitle, blogId }: CommentFormProps) {
  ...
  const resetForm = () => {
    const contactForm = document.getElementById("comment_form") as HTMLFormElement | null;
    if (contactForm) {
      contactForm.reset();
    }
  };

  const onSubmit = async (formData: CommentFormInputs) => {
    setLoading(true);
